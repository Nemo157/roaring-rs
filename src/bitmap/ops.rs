...
impl RoaringBitmap {
    ...
    pub fn intersect_with(&mut self, other: &RoaringBitmap) {
        retain_mut(&mut self.containers, |container| {
            match other.containers.binary_search_by_key(&container.key, |c| c.key) {
                Ok(loc) => {
                    container.intersect_with(&other.containers[loc]);
                    container.len != 0
                }
                Err(_) => false,
            }
    ...
    pub fn difference_with(&mut self, other: &RoaringBitmap) {
        retain_mut(&mut self.containers, |container| {
            match other.containers.binary_search_by_key(&container.key, |c| c.key) {
                Ok(loc) => {
                    container.difference_with(&other.containers[loc]);
                    container.len != 0
                }
                Err(_) => true,
            }
